...
const server = http.createServer((req, res) => {
  ...
  fs.stat(filePath, (err, stats) => {
    if (err) {
      // File doesn't exist - try index.html for directories
      if (req.url.endsWith('/')) {
        const dirIndex = path.join(filePath, 'index.html');
        fs.stat(dirIndex, (err2) => {
          if (err2) {
            res.writeHead(404, { 'Content-Type': 'text/html' });
            res.end(`<h1>404 - File Not Found</h1><p>Path: ${req.url}</p>`);
            return;
          }
        ...
        res.writeHead(404, { 'Content-Type': 'text/html' });
        res.end(`<h1>404 - File Not Found</h1><p>Path: ${req.url}</p><p>Resolved: ${resolvedPath}</p>`);
      }
      return;
    }
    ...
    if (stats.isDirectory()) {
      ...
      fs.stat(dirIndex, (err2) => {
        if (err2) {
          res.writeHead(404, { 'Content-Type': 'text/html' });
          res.end(`<h1>404 - File Not Found</h1><p>Directory has no index.html</p>`);
          return;
        }
        serveFile(dirIndex, res);
      });
